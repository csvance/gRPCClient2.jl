...
import Base.wait, Base.reset, Base.notify

abstract type gRPCException <: Exception end

struct gRPCNetworkException <: gRPCException
    message::String
end
...
function read_callback(
    ...
    try
        req = unsafe_pointer_to_objref(req_p)::gRPCRequest
        # TODO: do this without needing a copy
        buf = read(req.request, size*count)
        n = length(buf)
...
mutable struct gRPCRequest
    ...
    function gRPCRequest(grpc, url, request; deadline=10, keepalive=60)
        ...
        req = new(easy_handle, grpc.multi, http_url, request, IOBuffer(), UInt32(0), Event(), zeros(UInt8, CURL_ERROR_SIZE), Vector{String}())
...
function grpc_unary(grpc, url, request, TResponse; deadline=10, keepalive=60)
    ...
    try
        ...
        req.code != CURLE_OK && throw(gRPCNetworkException("libCURL returned easy request code $(req.code)"))
    ...
    try
        ...
        if action in (CURL_POLL_IN, CURL_POLL_OUT, CURL_POLL_INOUT)
            ...
            if sock in keys(WATCHERS)
                ...
                watch_change = watcher.fdw.readable != readable || watcher.fdw.writable == writable
